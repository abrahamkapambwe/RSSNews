    ...
    public class Pet
    {
        ...
        public static void PutPhoto(string domainName, string itemName, string bucketName, string fileName, Stream fileContent, bool isPublic, AmazonSimpleDBClient sdbClient, AmazonS3Client s3Client)
        {
            ...
            putAttrRequest.WithAttribute(new ReplaceableAttribute
                {
                    Name = "PhotoThumbUrl",
                    Value = String.Format(Settings.Default.BucketNameURL, String.Format(Settings.Default.BucketName, HttpContext.Current.User.Identity.Name, itemName), fileName),
                    Replace = true
                });
            ...
            if (isPublic)
            {
                DomainHelper.CheckForDomain(Settings.Default.RssNewsDomainName, sdbClient);
                putAttrRequest.DomainName = Settings.Default.RssNewsDomainName;
                sdbClient.PutAttributes(putAttrRequest);
            }
        }
        ...
        public void Put(string domainName, string itemName, bool isPublic, AmazonSimpleDBClient sdbClient)
        {
            ...
            {
                DeleteAttributesRequest deleteAttributeRequest = new DeleteAttributesRequest().WithDomainName(Settings.Default.RssNewsDomainName).WithItemName(itemName);
